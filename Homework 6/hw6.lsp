...
(defun at-most-one-color (n c k)
  (cond ((= c k) (list (list (* n k) (- (* n k)))))
    (t (one-from-many (at-least-one-color n c k))))
  )

; one-from-many is a helper function for at-most-one-color. It takes a list colors that contains all
; the possible colors for the node and then returns a list of clauses that represent that the node
; can only get at most one color from the set of possible colors.

(defun one-from-many (colors)
  (cond ((<= (len colors) 1) NIL)
    (t (append (permutations (- (car colors)) (cdr colors)) (one-from-many (cdr colors))))))

; permutations is a helper function for one-from-many. It takes an element and a list and returns all
; the lists of two elements permutations with the element and the negation of the elements in the list.

(defun permutations (x y)
  (cond ((NULL y) NIL)
    (t (append (list (list x (- (car y)))) (permutations x (cdr y))))))

; len is a helper function for one-from-many. It offers the functionality of the length function by
; returning the length of a list passed to it.
...
; generate-edge-clauses-helper is a helper function for generate-edge-clauses. It takes two edges, k,
; and an extra argument c which is the lower limit for the color indexes. This extra argument allows
; this function to be called recursively and it returns the result as specified by generate-edge-clauses

(defun generate-edge-clauses-helper (e c k)
  (cond ((< k c) NIL)
    (t (append (list (list (- (node2var (car e) c k))
      (- (node2var (second e) c k)))) (generate-edge-clauses-helper e (+ c 1) k)))))
